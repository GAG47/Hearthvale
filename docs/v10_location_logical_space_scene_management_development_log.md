...
`LogicalLocationCompiler` headless 编译所有 TileMapLayer。重叠 Tile 中任一层 blocked 即使该 Cell blocked；所有实际参与 Tile 的最小包围 Rect 成为 bounds，bounds 外和 bounds 内没有 Logical Tile 的孔洞默认 blocked。输出 `LogicalLocationData` 使用 bounds 映射到连续 `PackedByteArray` / `PackedInt32Array`，支持 O(1) walkability 与 movement-cost 查询，不为每个 Cell 建立 Node 或 Resource。

烘焙产物还保存 Entry / Exit、source fingerprint 和 compiler version。Entry 包含稳定 `entry_id`、Cell、Location-local position 与 facing；边仍只引用目标 Location 的 `entry_id`，Entry 不复制来源连接。产物不保存 Entity、State、Spatial Index、Use Slot 实际位置、Navigation、Reservation、Representation 或 Scene Node。
...
Preflight 对每个 Location 独立判断：未变化时跳过，缺失、指纹变化、版本变化或数据形状错误时只重烘焙对应产物。编译或保存失败会返回非零退出码，LocationSpace 也拒绝加载缺失或过期产物，因此不会静默继续使用旧 Static Grid。

启用的 `Location Bake Preflight` EditorPlugin 通过 Godot `_build()` 钩子在编辑器启动游戏前自动调用同一 Compiler；失败会返回 `false` 并阻止运行。命令行测试与构建使用下面的同一 headless 入口，因此不需要靠开发者记住逐个手工重烘焙 Location。
...
新增 `LocationSpace` Autoload，在 Scene 生命周期之外加载三个 LogicalLocationData，并为每个地点建立 `LogicalLocation`。它提供正式查询：
...
新增 `UseSlotDefinition`，表达 `relative_cell`、`required_facing` 和 `supported_actions`；坐标原点是 Entity footprint 左上角。床配置了明确的 sleep Slot：位于床左侧上格，Actor 必须面向右。没有显式 Slot 的储物箱与告示牌根据 footprint 外沿生成默认相邻 Slot，并过滤越界、静态 blocked 或被其他 blocking Entity 占据的位置。
...
- V10 专项 53 项检查通过，覆盖 headless 编译、bounds / holes / costs、指纹和 compiler version、Entry / Exit、未加载 Scene 的静态查询、床的两格 Occupancy、`cell → entity`、Current Walkability、Action 查询、显式与默认 Use Slot、统一移动与索引更新、Entry facing、Representation 恢复以及 Scene 卸载后的离屏查询；
