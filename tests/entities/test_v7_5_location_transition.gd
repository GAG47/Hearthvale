...
func _run_tests() -> void:
	...
	_expect_prepare_failure(
		game, controller, state_registry, player,
	...
	var real_yard_edge := location_registry.get_edge(tavern_id, &"back_door")
	...
	var yard_state := state_registry.get_location_state(yard_id)
	...
	var state_count := state_registry.get_entity_states().size()
	var location_state_count := state_registry.get_location_states().size()
	var chest_state := chest.furniture_state
	var openable_state := chest.get_openable_state()
	openable_state.is_open = true
	var old_tavern := game.get("current_location") as LocationScene
	...
	_expect(
		state_registry.get_entity_states().size() == state_count
		and state_registry.get_location_states().size() == location_state_count,
	...
	var return_edge := location_registry.get_edge(yard_id, &"tavern_door")
	var old_yard := yard
...
func _expect_prepare_failure(
	...
	var snapshot := _capture_world(game, controller, state_registry, player)
	...
	_expect(
		_capture_world(game, controller, state_registry, player) == snapshot,
		"%s failure must leave active world facts unchanged." % case_name
	)
	var old_location: LocationScene = snapshot["current_location"]
